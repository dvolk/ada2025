...
```bash
sudo apt update
sudo apt -y install --no-install-recommends pkg-config build-essential libvirt-clients virtinst libvirt-dev python3-openstackclient python3-glanceclient libpq-dev libvirt-daemon-system
```
...
ADA2025_EMAIL_LOGIN_SECRET_KEY # email login (passwordless) encoding key
ADA2025_SHARE_TOKEN_SECRET_KEY # share token encoding key
...
ADA2025_INSTANCE_IDENTIFIER # defaults to name of machine
```
